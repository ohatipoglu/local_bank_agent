...
class TestSettings:
    ...
    def test_temperature_validation(self):
        ...
        # Temperature too low
        with patch.dict(os.environ, {"LLM_TEMPERATURE": "-0.1"}):
            with pytest.raises(Exception):
                Settings()
    ...
    def test_audio_size_validation(self):
        ...
        # Valid size
        with patch.dict(os.environ, {"MAX_AUDIO_SIZE_MB": "25"}):
            settings = Settings()
            assert settings.MAX_AUDIO_SIZE_MB == 25

        # Size too large
        with patch.dict(os.environ, {"MAX_AUDIO_SIZE_MB": "51"}):
            with pytest.raises(Exception):
                Settings()

        # Negative size
        with patch.dict(os.environ, {"MAX_AUDIO_SIZE_MB": "-1"}):
            with pytest.raises(Exception):
                Settings()
    ...
    def test_rate_limit_validation(self):
        ...
        # Negative rate limit
        with patch.dict(os.environ, {"RATE_LIMIT_REQUESTS": "-5"}):
            with pytest.raises(Exception):
                Settings()
    ...
    def test_model_name_validation(self):
        ...
        # Valid model name
        with patch.dict(os.environ, {"LLM_MODEL_NAME": "gemma:7b"}):
            settings = Settings()
    ...
    def test_base_url_validation(self):
        ...
        with patch.dict(os.environ, {"LLM_BASE_URL": "localhost:11434"}):
            with pytest.raises(Exception):
                Settings()
